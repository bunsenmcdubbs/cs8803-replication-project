...
for i in range(1, n+1):
	for j in range(1, n+1):
		...
		F.variables.add(obj=[0.4], names=['neg'+str(i)+'_'+str(j)], types='B')
		#my_lin.append(cplex.SparsePair(ind=['neg'+str(i)+'_'+str(j)], val=[1]))
		F.variables.add(obj=[0.2], names=['neu'+str(i)+'_'+str(j)], types='B')
		
		index = ['pos'+str(i)+'_'+str(j), 'neg'+str(i)+'_'+str(j), 'neu'+str(i)+'_'+str(j)]
		my_lin.append(cplex.SparsePair(ind=index, val=[1,1,1]))
		my_sense.append('E')
		my_rhs.append(1)

F.linear_constraints.add(lin_expr=my_lin, rhs=my_rhs, senses=my_sense)
